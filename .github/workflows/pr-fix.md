...
jobs:
  guard:
    # Dispatch-only workflow that injects the push PAT. A manual dispatch may
    # select any ref, so require the default branch; the orchestrator always
    # dispatches this workflow on the default branch.
    if: >-
...
- MCP bearer: the config adapter writes the gateway's MCP `headers` (including
  ...
  only unlocks the gateway, which enforces the same tool guard policies. The
  staging step appends `opencode.jsonc` and `.opencode/` to the checkout-local
  `.git/info/exclude`, so a target branch that does not carry the root
  `.gitignore` rules cannot leak the file through `git add -A`.
...
2. If there are concrete, actionable issues, address each one (file:line + the
   ...
   commit so the autosquash cannot leave an unfoldable `fixup!` behind:
     ...
     for p in opencode.jsonc .opencode/; do
